...
$listuser=fopen('listapprenants.txt','r');
echo'<table border=1px class="table table-dark" >';
 ...
 <th>Mail</th>
 <th>Promotion</th>
 <th>Décision adminidtrative</th>
 
</tr>';

while (!feof($listuser)) {
$ligne=fgets($listuser);
$col=explode(";",$ligne);
echo '<tr>';
for($i=0;$i<7;$i++){
  echo "<td>".$col[$i]."</td>";
...
echo "<td><a classe='button' href='gestdecision.php?mail=$col[5]'> <button";
if($col[8]=='exclus(e)') {
   echo" style='background-color:red' >$col[8]</button></a></td>";
}
elseif($col[8]=='non exclus(e)')
{
    echo" style='background-color:green' >$col[8]</button></a></td>";   
}
 echo '</tr>';
